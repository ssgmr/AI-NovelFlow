# shot-audio-generation Specification

## Purpose
定义基于分镜台词和角色音色生成语音音频的能力要求，支持单个分镜和批量章节音频生成。
...
系统 SHALL 提供 API 端点批量生成章节所有分镜的角色台词音频。
...
### Requirement: 音频文件存储路径

生成的音频文件 SHALL 按规范路径存储。

#### Scenario: 音频文件命名规范
- **WHEN** 音频生成成功
- **THEN** 文件名 SHALL 使用格式：`story_{novel_id}/shot_audio/shot_{shot_id}_{character_name}.flac`

### Requirement: 音频生成任务类型

系统 SHALL 支持 `character_audio` 任务类型用于分镜角色台词音频生成。
