    ...
    try:
        ...
        if shot_characters:
            ...
            for char_name in shot_characters:
                ...
                if character and character.image_url:
                    full_path = _url_to_local_path(character.image_url)
                    if full_path:
            ...
            if character_images:
                merged_path = _merge_character_images(novel_id, chapter_id, shot_index, character_images)
        ...
        if shot_scene:
            ...
            if scene and scene.image_url:
                full_path = _url_to_local_path(scene.image_url)
                if full_path:
...
async def generate_shot_video_task(
    ...
    try:
        ...
        character_reference_path = None
        if shot_image_url:
            full_path = _url_to_local_path(shot_image_url)
            if full_path:
...
async def generate_transition_video_task(
    ...
    try:
        ...
        # 转换URL为本地路径
        first_video_path = _url_to_local_path(first_video_url)
        second_video_path = _url_to_local_path(second_video_url)
...
def _load_chinese_font(size: int) -> ImageFont:
    """加载中文字体"""
    font_paths = [
        # macOS
